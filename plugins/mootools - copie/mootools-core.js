...
}}return c;},substitute:function(a,b){return this.replace(b||(/\ ? { ([ ^ {}] + )
        }
        /g),function(d,c){if(d.charAt(0)=="\"){return d.slice(1);}return(a[c]!=undefined)?a[c]:"";
...
case false:return"null ";}return null;},decode:function(string,secure){if($type(string)!="string "||!string.length){return null;}if(secure&&!(/^[,:{}[]0-9.-+Eaeflnr-u nrt]*$/).test(string.replace(/\./g,"@").replace(/" [ ^ "\nr]*" / g, ""))) {
            return null;
        }
        return eval("(" + string + ")");
    }
