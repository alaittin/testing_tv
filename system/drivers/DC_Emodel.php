 ...
 * Class DC_Emodel
 *
 * Provide methods to modify the database using Emodels.
 * @copyright  Leo Feyer 2005-2010
 * @author     Leo Feyer <http://www.typolight.org>
 * @package    Controller
 */
class DC_Emodel extends DataContainer implements listable, editable
{
	...
	protected $table_list_for_validation = array('tl_tvfi_bdf_bo_societe_vendeuse');
	...
	public function __construct($strTable)
	{
		...
		// NEW
		$this->emodel = new $GLOBALS['TL_DCA'][$this->strTable]['config']['emodel']($this->intId);
		...
		if (is_array($GLOBALS['TL_DCA'][$this->strTable]['config']['onload_callback']))
		{
			foreach ($GLOBALS['TL_DCA'][$this->strTable]['config']['onload_callback'] as $callback)
			{
				if (is_array($callback))
				{
					...
					$this->$callback[0]->$callback[1]($this->emodel);
				}
			}
		}

#		// Call before_save calbacks 
#		if (is_array($GLOBALS['TL_DCA'][$this->strTable]['config']['before_save']))
#		{
#			foreach ($GLOBALS['TL_DCA'][$this->strTable]['config']['before_save'] as $callback)
#			{
#				if (is_array($callback))
#				{
#					$this->import($callback[0]);
...
#			// if there is a before save callback function in Emodel then call that function
#			if (in_array($this->strTable,$this->table_list_for_validation))
#			{
#				$objModel = new $this->emodel($this->intId);
#				if (count( $objModel->beforeSave)){
#					foreach ($objModel->beforeSave as $callback)
#					{
#						$this->import($callback[0]);
#						$this->$callback[0]->$callback[1]($this);
#					}
...
#		if (in_array($this->strTable,$this->table_list_for_validation))
# 	{
# 		$objModel = new $this->emodel();
# 		#$objModel->save();
# 		echo "<pre>".var_dump($objModel->beforeSave);
# 		if (count( $objModel->beforeSave)){
# 		#	echo "<pre>".var_dump($objModel->beforeSave);
# 		}
# 		#echo var_dump($this->emodel->save()); # call save of emodel so if there is call back check it
# 	}
...
# 	$objModel = new $this->emodel();
# 	$results = $objModel->getAll('id', NULL,10);
# 	
# 	foreach($results as $result)
# 	{
# 		echo $result->fsv_nom .'<br/>'; 
# 	
# 	} 
